...
public class Day8 extends BaseTest {
	...
	@Test public void runGold() throws Exception {
		...
		final String finalLayer = getFinalLayer(layersList);
	...
	public String getFinalLayer(final ArrayList<String> layers) {
		final int length = layers.get(0).length();
		String finalLayer = "";
		System.out.println("layers size: " + layers.size());
		for (int i = 0; i < length; i++) {
			boolean found = false;
			for (int j = 0; j < layers.size(); j++) {
				final char currentChar = layers.get(j).charAt(i);
	...
	public ArrayList<String> readLayers(final String input, final int wide, final int tall) {
	...
	public String findLayer(final String input, final int wide, final int tall) {
		String minLayer = "";
		int minMatch = Integer.MAX_VALUE;
		for (int i = 0; i < input.length(); i = i + wide * tall) {
			final String currentLayer = input.substring(i, i + wide * tall);
			final int currentMatch = StringUtils.countMatches(currentLayer, "0");
