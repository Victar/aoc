...
public class Day17 extends BaseTest {
	...
	public Set<Cord> doCheck(Set<Cord> activeSet, int start, int end) {
			...
			for (int j = start; j < end; j++) {
				for (int k = start; k < end; k++) {
					for (int w = start; w < end; w++) {
						Cord toCheck = Cord.of(i, j, k, w);
						int nbr = checkNeighbors(toCheck, activeSet);
						boolean contains = activeSet.contains(toCheck);
						if (!contains && nbr == 3) {
			...
			for (int j = 0; j < dY.length; j++) {
				for (int k = 0; k < dZ.length; k++) {
					for (int w = 0; w < dW.length; w++) {
						if (i != 1 || j != 1 || k != 1 || w != 1) {
							Cord neighborToCheck = Cord.of(cord.x + dX[i], cord.y + dY[j], cord.z + dZ[k], cord.w + dZ[w]);
